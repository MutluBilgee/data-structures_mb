package week1AssignmentSolutionsLinkedLists;
...
public class MySinglyLinkedList {
    ...
    void addFirst(int data){
        ...
        else { // add element to first position
            node.next=head;
            head=node;
        }
        size++;
    ...
    public int getKthFromLast(int k){
        ...
        Node ptr1=head;
        Node ptr2=head;
        // move pointer2 k times
        for(int i=0;i<k-1;i++) {ptr2=ptr2.next;
        if (ptr2==null) return -1;
        }
        // move pointers simultaneously until pointer 2 hits last item
